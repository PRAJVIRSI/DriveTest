    ...
    const load = (status) => {
        $.ajax({
            ...
            for (const user of result.users) {
                const appointment = user.testType === 'G2' ? user.appointmentId : user.appointmentIdG;
                const item = `<div class="row gx-4">
                                ...
                                <div class="col-md-3 my-2">${appointment.date.substring(0,10) + ' ' + appointment.time}</div>
                                <div class="col-md-1 my-2">${user.testType}</div>
                                <div class="col-md-2 my-2"><button type="button" style="margin-top:-5px;">edit</button></div>
            ...
            $('#driverList').off('click', 'button').on('click', 'button', function() {
                const box = $('#commentBox');
                box.empty();

                const firstName = $(this).parent().prev().prev().prev().prev().prev().text();
