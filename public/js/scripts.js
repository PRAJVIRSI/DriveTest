...
const checkUserInfoForm = function () {
    ...
    if (houseNoValue === "") {
        houseNo.next().text("This field cannot be empty.");
        incorrect += 1;
    ...
    if (carMadeYearValue === "") {
        carMadeYear.next().text("This field cannot be empty.");
        incorrect += 1;
...
const checkEmptyField = function(selector) {
    ...
    if (value === "") {
        element.next().text("This field cannot be empty.");
        return 1;
    }
    element.next().text("");
    element.val(value);
    ...
    $('#userName').blur(function() {
        ...
        if (name.length > 0) {
            $.ajax({
                ...
                if (result.exist) {
                    $('#userName').next().text("Username already exists, please enter another one");
    ...
    fetch(`/users/timeslot?date=${date}`)
        ...
        .then(json => {
            ...
            for (let slot of slots) {
                ...
                availableSlots.append($(`<button type="button" class="${style}" id="${slot.id}">${slot.time}</button>`));
            }
            if (slots.length > 0) {
                availableSlots.children().each(function () {
                    $(this).click(() => {
                        if ($(this).attr('class') === 'btn-time') {
                            availableSlots.find('.btn-time-selected').attr('class', 'btn-time');
                ...
                availableSlots.append('There is no available timeslot');
            }
        });
};
...
const g2PageLoad = function () {
    ...
    appointmentDate.change(function () {
        if (isValidDate(appointmentDate)) {
            const inUseSlot = adg2.val() === appointmentDate.val() ? {
                id: ssid.val(),
                time: atg2.val(),
                inUse: true
            } : undefined;
            availableSlots.empty();
            availableSlots.append('loading ... ...');
            loadTime(availableSlots, selectedSlotId, appointmentDate.val(), inUseSlot);
        }
    });
    ...
    $("#g2Form").submit( event => {
        ...
        if (licenseNo.length > 0) {
            ...
            if (licenseNoValue === "") {
                licenseNo.next().text("This field cannot be empty.");
                incorrect += 1;
            } else {
                licenseNo.next().text("");
            }
        ...
        let date = new Date(dateParts.join("/"));
        if (dobValue === "") {
            dob.next().text("This field cannot be empty.");
            incorrect += 1;
        } else if (dateParts.length !== 3
            || isNaN(dateParts[0])
            || date === "Invalid Date") { // check valid date format
    ...
    $('#carPic_g2').mouseenter( () => {
        $('#carPic_g2_cover').css('display', 'flex');
    });
    $('#carPic_g2_cover').mouseleave( () => {
        $('#carPic_g2_cover').css('display', 'none');
    });
    $('#carPic_g2_edit').click(() => {
        $('#carPic_g2').css('visibility', 'hidden');
        $('#carPic_g2_cover').css('display', 'none');
        $('#carPic').parent().css('display', 'flex');
    });


    $('#licensePic_g2').mouseenter( () => {
        $('#licensePic_g2_cover').css('display', 'flex');
    });
    $('#licensePic_g2_cover').mouseleave( () => {
        $('#licensePic_g2_cover').css('display', 'none');
    });
    $('#licensePic_g2_edit').click(() => {
        $('#licensePic_g2').css('visibility', 'hidden');
        $('#licensePic_g2_cover').css('display', 'none');
        $('#licensePic').parent().css('display', 'flex');
    });
};
...
const gPageLoad = function () {
    ...
    appointmentDate.change(function () {
        if (isValidDate(appointmentDate)) {
            const inUseSlot = adg.val() === appointmentDate.val() ? {
                id: ssid.val(),
                time: atg.val(),
                inUse: true
            } : undefined;
    ...
    $('#carPic_g').mouseenter( () => {
        $('#carPic_g_cover').css('display', 'flex');
    });
    $('#carPic_g_cover').mouseleave( () => {
        $('#carPic_g_cover').css('display', 'none');
    });
    $('#carPic_g_edit').click(() => {
        $('#carPic_g').css('visibility', 'hidden');
        $('#carPic_g_cover').css('display', 'none');
        $('#carPic').parent().css('display', 'flex');
    });
};
        ...
        return (date, $timeContainer, $selectedSlots) => {
            fetch(`/admin/timeslot?date=${date}`)
                ...
                .then(json => {
                    $timeContainer.empty();
                    $selectedSlots.val('');
                    const slots = defaultSlots.filter(s => !json['slots'].includes(s.time));
                    for (let slot of slots) {
                        $timeContainer.append($(`<button type="button" class="btn-time">${slot.time}</button>`));
                    }
                    $timeContainer.children().each(function () {
                        $(this).click(() => {
                            if ($(this).attr('class') === 'btn-time') {
                                ...
                                $selectedSlots.val($selectedSlots.val().replace($(this).text() + ',', ''));
                            }
                        })
                    });
                });
